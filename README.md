...
共享同步命令会创建 Python 3.11 `.venv`、安装 `requirements-lock.txt`、执行 `npm ci`，并用锁文件摘要跳过没有变化的重复安装。拉取新代码后也可以直接重新运行。只读环境诊断命令为：

```powershell
.\.venv\Scripts\python.exe scripts\dev_env.py doctor --strict
```

如果 `python` 命令不可用，批处理会自动尝试 `py -3.11`。项目使用 Python 3.11；前端推荐 Node 22 LTS。

## 本地 Web 仪表盘

Web 仪表盘是本地复核控制台，用来查看最新一次自动信号运行是否可以进入人工交易复核。首屏展示结构化摘要、质量门槛、阻塞修复中心、订单摘要和报告链接；同时提供受控运行按钮，只能启动白名单后台任务：
...
后台任务状态写入 `outputs/dashboard_jobs/`，日志写入 `outputs/logs/dashboard_job_*.log`。仪表盘不会编辑策略配置、推广候选信号或强制正式输出。
任务卡片会显示结构化运行进度；阻塞修复中心会把当前 blocker 映射成可一键处理的白名单动作或人工处理建议。修复 `daily_basic` 后仍需要重跑自动信号，才能刷新最终复核结论。
“运行前预检查”会在重跑自动信号前只读检查目标交易日、数据健康、点时治理、因子新鲜度和账户/持仓输入；它不会下载数据、重算因子或写入交易信号。
因子目标日覆盖率门槛为 99%；达到门槛且剩余股票已确认停牌或无新行情时，预检查按通过展示，不会再用最老单只股票的日期制造全局过期警告。
任务运行中可以在前端点击“停止当前任务”，后端会停止对应的 dashboard 后台任务及其子进程，并把任务状态记录为 `cancelled`，日志仍保留用于排查。

人工交易单中的股票名称和股票代码可以点击进入站内股票详情。详情页通过后端固定调用 Tushare `rt_k` 显示当前价、涨跌幅、昨收、开盘、最高、最低、成交量和行情获取时间，并提供独立刷新按钮；浏览器不能指定其他 Tushare 接口。实时接口不可用时，页面自动降级到 `data/raw/<TS_CODE>.csv` 的最新本地日线收盘价，同时显示行情日期和“非实时”标识，避免把陈旧价格误认为实时行情。

“交易执行”工作区支持从最新正式 `fill_feedback_*.csv` 模板录入真实成交状态、成交股数、价格和费用。股票代码、买卖方向和计划股数由后端锁定，不能在前端修改；更新真实持仓前必须先通过校验并再次确认。候选成交模板不会进入该工作区，校验失败也不会修改 `config/current_holdings.csv`。成功更新后会保存成交模板并写入 `outputs/fill_apply_audit_*.json` 审计文件。

“账户与持仓”工作区用于维护总资产、可用现金、单股仓位上限、普通/科创板交易手数和当前真实持仓。保存前会检查股票代码、重复持仓、负数/小数股数和交易手数倍数；确认保存时会先把已有 `account.yaml` / `current_holdings.csv` 复制到 `outputs/account_backups/`，再原子替换正式文件。该页面不会读取或展示 Tushare token，也不能修改策略配置和文件路径。
...
脚本会先调用共享环境同步命令，再启动 FastAPI 后端和 React/Vite 前端，检查 `http://127.0.0.1:8000/api/health` 与 `http://127.0.0.1:5173` 可访问，然后打开浏览器。锁文件没有变化时同步步骤会快速跳过。
...
Web 端现已提供独立的“量化操作中心”，覆盖增量行情、点时数据、财务数据、历史股票池、数据转换、因子计算、因子诊断、参数优化、真实化回测、研究诊断、基本面筛选和候选信号生成。所有操作均由后端映射到固定白名单命令，不允许前端提交任意脚本或 Shell 参数。

重复日常运行会复用已经覆盖目标日期的价格、Qlib 和因子缓存：行情更新确认 `written=0` 时跳过约数分钟的数据转换；因子缓存从第一天真实可用价格开始即可复用，即使配置的研究起点更早。Windows 默认把 Qlib worker 限制为 4，并关闭特征计算不需要的学习处理器，避免全市场 Alpha158 计算产生多 GiB 临时复制；Ubuntu 高内存服务器可在 `settings.local.yaml` 覆盖 `qlib.kernels`。

需要调整运行规模时，可以展开工作流卡片的“配置”区域。Web 当前支持受控的目标日期、批次大小、等待时间、缓存强制刷新、walk-forward 时间窗口、最长运行时间和研究样本数等参数；高级区还提供风险参数精炼、市场状态混合探针、调仓漂移探针和有界年度状态路由网格。文件路径、输出位置和任意命令参数不会开放给浏览器。

Ubuntu 或云服务器部署请参考 [docs/web-deployment.md](docs/web-deployment.md)。云端部署默认仍应监听 `127.0.0.1`，通过 SSH 隧道、私有 VPN 或带认证/TLS 的反向代理访问；不要把拥有任务运行和持仓更新权限的端口直接暴露到公网。
...
Web 端真实浏览器测试默认使用系统 Chrome，覆盖工作区导航、任务启动/停止、成交回填、账户保存和移动端布局：
...
Ubuntu CI 可先运行 `npx playwright install --with-deps chromium`，再使用 `PLAYWRIGHT_CHANNEL=chromium npm run test:e2e`。

GitHub Actions 会在 Windows 和 Ubuntu 的 Python 3.11 环境运行完整回归测试；独立的 Node 22 任务执行前端类型检查/生产构建，并在 Ubuntu Chromium 中运行 Playwright。
